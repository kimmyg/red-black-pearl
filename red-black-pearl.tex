...
Deletion in binary search trees is fairly straightforward. First, locate the node that contains the value to delete. If the node has no right subtree, replace it with its left subtree. If it has a right subtree, replace its value with the minimum value of its right subtree and remove that value from the right subtree.\footnote{An alternative is to distinguish left subtrees and use the maximum element. By considering right subtrees, we get a \texttt{min\_element} function for free, which is critical for priority queues.} Thus, deletion is recursive.
